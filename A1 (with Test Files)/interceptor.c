 ...
 *
 */
asmlinkage void my_exit_group(struct pt_regs reg) {
    // Remove the exiting pid from all lists.
...
asmlinkage long interceptor(struct pt_regs reg) {
    // We need a spinlock to avoid any misprint.
    spin_lock(&my_table_lock);
    
    // Check if the syscall is monitored for the current->pid.
    // ax (or EAX) saves the syscall number.
    // We have 2 cases here: 1) the syscall monitors all pids, which is monitored = 2;
    //                       2) check for a specific pid.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    // Part 1: Input error checks, i.e. EINVAL, EPERM, and EBUSY.
    // Part 2: Execution based on cmd.
    
    // Need consider the cases of -ENOMEM in the Part 2. ISO C90 forbids mixed 
	// declarations and codes, so we have to declare this variable at the top.
    int ENOMEMVal;
    
    // Part 1: Error checks.
    // This is not included in the docstring, but it is safer to check if cmd 
    // is valid.
    ...
    // Check if pid is valid for the monitoring commands.
    if (pid < 0 || (pid != 0 && pid_task(find_vpid(pid), PIDTYPE_PID) == NULL)) {
        
        return -EINVAL;
    }
    
    // Check if the caller has the right permissions.
    // For INTERCEPT/RELEASE.
    if ((cmd == REQUEST_SYSCALL_INTERCEPT || cmd == REQUEST_SYSCALL_RELEASE)
        && current_uid() != 0) {
        
        return -EPERM;
        
        // For START/STOP MONITORING.
    } else if ((cmd == REQUEST_START_MONITORING || cmd == REQUEST_STOP_MONITORING)
               && (current_uid() != 0
                   && (pid == 0 || check_pids_same_owner(current->pid, pid) != 0))) {
                   
                   return -EPERM;
               }
    
    // Need to lock my_table, since different processes may read from or write 
    // to it at the same time.
    spin_lock(&my_table_lock);
    
    // Check if the context of commands is correct.
    if ((cmd == REQUEST_SYSCALL_RELEASE || cmd == REQUEST_START_MONITORING)
        ...
        // Due to readability, we did not combine this case into the above one.
        // If a stop-monitoring-request comes in for a specific PID, for a 
        // syscall that monitors all PIDs, then we assume this is an error.
    } else if (cmd == REQUEST_STOP_MONITORING
               && (table[syscall].intercepted == 0
                   || check_pid_monitored(syscall, pid) == 0
                   || (pid != 0 && table[syscall].monitored == 2))) {
                   spin_unlock(&my_table_lock);
    ...
    // Check for -EBUSY conditions.
    // If some processes are being monitored (monitored == 1), and pid = 0 is passed,
    // then this is not an error case.
    if ((cmd == REQUEST_SYSCALL_INTERCEPT && table[syscall].intercepted != 0)
        || (cmd == REQUEST_START_MONITORING
            && ((pid != 0 && check_pid_monitored(syscall, pid) == 1) || table[syscall].monitored == 2))) {
            spin_unlock(&my_table_lock);
    ...
    // Part 2: cmd execution.
    if (cmd == REQUEST_SYSCALL_INTERCEPT) {
        spin_lock(&my_table_lock);
        spin_lock(&sys_call_table_lock);
        
        // Save the original sys_call.
        table[syscall].f = sys_call_table[syscall];
        
        // Setup sys_call's info in my_table.
        ...
        // Put the original sys_call back.
        set_addr_rw((unsigned long) sys_call_table);
        ...
        // Check if there is any available memory, when adding a pid.
        ENOMEMVal = add_pid_sysc(pid, syscall);
        if (ENOMEMVal != 0) {
            spin_unlock(&my_table_lock);
        ...
        if (pid == 0) {
            ...
            ENOMEMVal = del_pid_sysc(pid, syscall);
        }
...
static int init_function(void) {
    ...
    // Although if we do not call spin_lock_init, the init_function might
    // still work, it is safer to call spin_lock_init, because we do not
    // know what is saved/left on the memory.
    spin_lock_init(&sys_call_table_lock);
    spin_lock_init(&my_table_lock);
    spin_lock(&sys_call_table_lock);
    spin_lock(&my_table_lock);
    
    // Save the original custom syscall and exit_group.
    ...
    orig_exit_group = sys_call_table[__NR_exit];
    ...
    // Experiment: We tried to set orig_exit_group before setting
    // orig_custom_syscall, then the system always crash.
    sys_call_table[__NR_exit] = my_exit_group;
...
static void exit_function(void) {
    ...
    // Restore the original custom syscall and exit_group.
    set_addr_rw((unsigned long) sys_call_table);
    ...
    sys_call_table[__NR_exit] = orig_exit_group;
